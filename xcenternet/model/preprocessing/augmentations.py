...
from xcenternet.model.preprocessing.color import tf_py_blur, tf_py_contrast, tf_py_dropout
...
class HardAugmentation(Augmentation):
    def __init__(self, probability):
        ...
        self.augmentation_config.rotate90 = True
        self.augmentation_config.rotate_max = True
        self.augmentation_config.flip_vertical = False
        self.augmentation_config.flip_vertical = True
        self.padding_square = False

    def augment(self, image, bboxes):
        image, bboxes = random_augmentations(image, self.augment, bboxes=bboxes)
        image, bboxes = random_function_bboxes(image, bboxes, random_pad_to_square, 0.3)

        # This is pixel wise augmentation and bboxes will not change
